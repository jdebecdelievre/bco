...
def loss_calc(i, o, do, cl, model, params, coefs={}):
    ...
    #         #fc_loss = controlled_leaky_relu(i, _o_ * cl, bound)
    ...
    #     # grad_norm = (torch.abs(torch.square(_do_ / (model.input_std/model.output_std)).sum(dim=1) - 1.).T @ feasible) / N_feasible
    ...
    #         grd = grad(model._net(anchors).sum(), [anchors], create_graph=True)[0]
    #         grad_norm = grad_norm + torch.abs(1 - torch.square(grd / (model.input_std/model.output_std)).sum(dim=1)).mean()
    #         # grad_norm = grad_norm + torch.abs(torch.norm(grd / (model.input_std/model.output_std), dim=1) - 1.).mean()
    ...
    #     return loss, loss_dict
        ...
        feasible_class =  (fc_loss.T @ feasible) / N_feasible * (2 * (_i.size(1) + 1))# 2x(dim+1) to account for xStar and derivatives
        grad_norm = (torch.abs(1 - torch.square(_do_ / (model.input_std/model.output_std)).sum(dim=1)).T @ feasible) / N_feasible
        # grad_norm = grad_norm + torch.abs(model._net(_i - _do_ * _o_)).sum()
        ...
        if 'bounds' in params and params['sdf_regularization_anchors'] > 0:
            ...
            # sdf_reg = sdf_reg + 0.1*(anchout * model._net(anchors - grd * anchout)).sum()
    ...
    # Infeasible boundary
        ...
        if 'bounds' in params and \
            ...
            bound  = torch.tensor(params['bounds'])
            # upper_anchors = torch.rand(params['boundary_anchors'], _i.shape[1]) * (bound[1:] - bound[:1]) + bound[:1]
            upper_anchors = anchors.clone()
            lower_anchors = upper_anchors.clone()

            ivar = np.random.choice(_i.shape[1]-1, size=params['boundary_anchors'])
            for i, iv in enumerate(ivar):
                upper_anchors[i, iv] = bound[1][iv]
            boundary_reg = F.relu(-model._net(upper_anchors, reuse=True)).mean() 
            
            for i, iv in enumerate(ivar):
                lower_anchors[i, iv] = bound[0][iv]
            boundary_reg = boundary_reg + F.relu(-model._net(lower_anchors, reuse=True)).mean()
